...
import StudyWritePage from "@/components/study/studySpaceWrite";
...
async function getQuiz(studySpaceID: string) {
  ...
  const quizzes = data.StudySpaceQuizzes!.filter((quiz) => quiz.studyType == "WRITE");
  return quizzes[0];
}
...
const page = async ({ params }: { params: { studySpaceID: string, classID: string} }) => {
  ...
  return <StudyWritePage quiz={quiz} classID={params.classID} />;
