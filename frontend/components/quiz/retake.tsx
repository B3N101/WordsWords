...
export default function RequestRetakePage({ quiz }: Props) {
  ...
  const [requested, setRequested] = useState<boolean>(userWordsList.retakesRequested[miniSet]);
  const handleRetakeClick = () => {
    setRequested(true);
    upsertRetakesRequested(userWordsList.userId, userWordsList.wordsListListId, miniSet, true);
  }
