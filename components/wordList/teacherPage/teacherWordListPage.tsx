...
import {
  getAllUserWordsListProgresses,
  getListNameAndDueDate,
  getListSize,
} from "@/prisma/queries";
...
async function getFreshmenData(
  ...
  const studentWordsLists = userWordsLists.filter(
    (wordList) => wordList.user.role === "STUDENT",
  );
...
export async function TeacherWordListHeader({
  classId,
  wordListID,
}: {
  classId: string;
