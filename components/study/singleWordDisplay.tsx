...
export default function SingleWordDisplay({ studyWord, starred, setStarred, setUnstarred}: StarButtonProps) {
  ...
  return (
    <div className="flex group items-center justify-between border-2 border-[#ff6b6b] rounded-lg p-2">
     <div className="text-lg text-gray-600 font-semibold">{studyWord.word.word}</div>
     <div className="text-sm text-gray-600 m-auto">{studyWord.word.definition}</div>
      
      <Button variant="outline" onClick={() => handleStarClick(isStarred)} className="ml-auto float-right">
